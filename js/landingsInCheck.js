...
function landingInCheck(ii, nn) {
            ...
            while (n < 64) {
               ...
               if (markL < sqR && markR > sqL && markT < sqB && markB > sqT) { //check collisions marks/squares
                  if (gpP === "C") { //CASTLE check if type C
                     if (gpPn === "c" && n !== i - 16) {
                        if (n === i - 7 || n === i - 9 || WhiteCastlesInCheck.includes(i) || BlackCastlesInCheck.includes(n) || TotalBCastles === 1 || TotalWCastles === 1) { //check exceptions to special Castle's not in check diagonal moves
                           underBlackC(n);
                        }
                     }
                  } else if (gpP === "c") { //castle check if type c
                     if (gpPn === "C" && n !== i + 16) {
                        if (n === i + 7 || n === i + 9 || WhiteCastlesInCheck.includes(n) || BlackCastlesInCheck.includes(i) || TotalBCastles === 1 || TotalWCastles === 1) { //check exceptions to special Castle's not in check diagonal moves
                           underWhiteC(n);
                        }
                     }
                  } else if (tripleCheck === 0) {
                     if (gpP === "P") { //PAWN
                        if ((n === i - 9 && gpPn === "c") || (n === i - 7 && gpPn === "c")) { //check exceptions to special Castle's not in check diagonal moves
                           underBlackC(n);
                        }
                        ...
                        if (marksOnBoardToBishopNW.includes(marksOnBoard[m - 1])) { //NW
                           ...
                           while (r < lineSize) {
                              r++;
                              P = i - (1 + r * 9);
                              if (GhostPiecesPosition[P] !== "O" && r < lineSize) { //following line
                                 break;
                              }
                              if (r === lineSize) {
                                 if (GhostPiecesPosition[P] === "C" || GhostPiecesPosition[P] === "c") {
                                    if (gpP === gpP.toUpperCase() && GhostPiecesPosition[P] === GhostPiecesPosition[P].toLowerCase()) {
                                       underBlackC(n);
                                    } else if (gpP === gpP.toLowerCase() && GhostPiecesPosition[P] === GhostPiecesPosition[P].toUpperCase()) {
                                       underWhiteC(n);
                                    }
                                 }
                              }
   ...
   //console.log("landings count: " + countCheckmate + " W: " + WhiteLandingsInCheck.length  + " B: " + BlackLandingsInCheck.length);
